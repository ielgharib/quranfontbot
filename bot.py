...
def init_db():
    ...
    # نقل البيانات من ملفات JSON القديمة إذا وجدت
    if os.path.exists("responses.json"):
        with open("responses.json", 'r', encoding='utf-8') as f:
            responses = json.load(f)
            cursor.executemany('INSERT OR IGNORE INTO responses VALUES (?, ?)', responses.items())
        ...
        with open("users.json", 'r', encoding='utf-8') as f:
            ...
            for user_id, user_data in users.items():
                cursor.execute('INSERT OR IGNORE INTO users VALUES (?, ?, ?, ?)',
                             (user_id, user_data['name'], user_data.get('username'), user_data.get('first_seen', str(datetime.now()))))
        ...
        with open("stats.json", 'r', encoding='utf-8') as f:
            ...
            if 'total_users' in stats:
                for user_id in stats['total_users']:
                    cursor.execute('INSERT OR IGNORE INTO users VALUES (?, ?, ?, ?)',
                                 (user_id, "Unknown", None, str(datetime.now())))
            
            if 'total_groups' in stats:
                for group_id in stats['total_groups']:
                    cursor.execute('INSERT OR IGNORE INTO groups VALUES (?, ?)',
                                 (group_id, str(datetime.now())))
...
def save_responses(responses):
    ...
    cursor.executemany('INSERT INTO responses VALUES (?, ?)', responses.items())
    conn.commit()
    conn.close()
...
def load_stats():
    ...
    stats["total_users"] = {row[0] for row in cursor.fetchall()}
    
    cursor.execute('SELECT group_id FROM groups')
    stats["total_groups"] = {row[0] for row in cursor.fetchall()}
    
    cursor.execute('SELECT command, count FROM commands_used')
    stats["commands_used"] = {row[0]: row[1] for row in cursor.fetchall()}
    
    cursor.execute('SELECT user_id, time, chat_id, command FROM last_active')
...
def update_stats(update: Update, command: str = None):
    ...
    cursor.execute('''
    INSERT OR IGNORE INTO users (user_id, name, username, first_seen)
    VALUES (?, ?, ?, ?)
    ''', (user_id, update.effective_user.full_name, update.effective_user.username, str(datetime.now())))
    
    if update.effective_chat.type in ["group", "supergroup", "channel"]:
        cursor.execute('''
        INSERT OR IGNORE INTO groups (group_id, last_active)
        VALUES (?, ?)
        ''', (chat_id, str(datetime.now())))
    
    if command:
        cursor.execute('''
        INSERT INTO commands_used (command, count)
        VALUES (?, 1)
        ON CONFLICT(command) DO UPDATE SET count = count + 1
        ''', (command,))
    
    cursor.execute('''
    INSERT OR REPLACE INTO last_active (user_id, time, chat_id, command)
    VALUES (?, ?, ?, ?)
    ''', (user_id, str(datetime.now()), chat_id, command or "message"))
...
def save_users(users_data):
    ...
    for user_id, user_info in users_data["users"].items():
        ...
        ''', (user_id, user_info['name'], user_info.get('username'), user_info.get('first_seen', str(datetime.now()))))
    conn.commit()
    conn.close()
...
def load_user_messages():
    ...
    cursor = conn.cursor()
    cursor.execute('''
    SELECT message_id, user_id, user_name, username, message, timestamp, replied, reply_text, reply_timestamp
    FROM user_messages
    ''')
    messages = {
        str(row[0]): {
...
def save_user_messages(messages_data):
    ...
    for msg_id, msg in messages_data["messages"].items():
        cursor.execute('''
        INSERT INTO user_messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            int(msg_id), msg["user_id"], msg["user_name"], msg.get("username"), 
            msg["message"], msg["timestamp"], int(msg["replied"]), 
            msg.get("reply_text"), msg.get("reply_timestamp")
        ))
    conn.commit()
    conn.close()
